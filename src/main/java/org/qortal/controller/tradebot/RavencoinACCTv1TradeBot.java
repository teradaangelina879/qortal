 ...
 * </ul>
 */
public class RavencoinACCTv1TradeBot implements AcctTradeBot {

	private static final Logger LOGGER = LogManager.getLogger(RavencoinACCTv1TradeBot.class);
	...
	private static RavencoinACCTv1TradeBot instance;

	private final List<String> endStates = Arrays.asList(State.BOB_DONE, State.BOB_REFUNDED, State.ALICE_DONE, State.ALICE_REFUNDING_A, State.ALICE_REFUNDED).stream()
			.map(State::name)
			.collect(Collectors.toUnmodifiableList());

	private RavencoinACCTv1TradeBot() {
	}

	public static synchronized RavencoinACCTv1TradeBot getInstance() {
		if (instance == null)
			instance = new RavencoinACCTv1TradeBot();
	...
	public byte[] createTrade(Repository repository, TradeBotCreateRequest tradeBotCreateRequest) throws DataException {
		...
		byte[] creationBytes = RavencoinACCTv1.buildQortalAT(tradeNativeAddress, tradeForeignPublicKeyHash, tradeBotCreateRequest.qortAmount,
		...
		TradeBotData tradeBotData =  new TradeBotData(tradePrivateKey, RavencoinACCTv1.NAME,
	...
	public ResponseResult startResponse(Repository repository, ATData atData, ACCT acct, CrossChainTradeData crossChainTradeData, String xprv58, String receivingAddress) throws DataException {
		...
		byte[] messageData = RavencoinACCTv1.buildOfferMessage(tradeBotData.getTradeForeignPublicKeyHash(), tradeBotData.getHashOfSecret(), tradeBotData.getLockTimeA());
	...
	public void progress(Repository repository, TradeBotData tradeBotData) throws DataException, ForeignBlockchainException {
		...
		if (tradeBotState.requiresAtData) {
			...
			if (tradeBotState.requiresTradeData) {
				tradeData = RavencoinACCTv1.getInstance().populateTradeData(repository, atData);
				if (tradeData == null) {
	...
	private void handleBobWaitingForMessage(Repository repository, TradeBotData tradeBotData,
		...
		for (MessageTransactionData messageTransactionData : messageTransactionsData) {
			...
			RavencoinACCTv1.OfferMessageData offerMessageData = RavencoinACCTv1.extractOfferMessageData(messageData);
			...
			int refundTimeout = RavencoinACCTv1.calcRefundTimeout(messageTimestamp, lockTimeA);
			...
			byte[] outgoingMessageData = RavencoinACCTv1.buildTradeMessage(aliceNativeAddress, aliceForeignPublicKeyHash, hashOfSecretA, lockTimeA, refundTimeout);
	...
	private void handleAliceWaitingForAtLock(Repository repository, TradeBotData tradeBotData,
		...
		int refundTimeout = RavencoinACCTv1.calcRefundTimeout(recipientMessageTimestamp, lockTimeA);
		...
		byte[] messageData = RavencoinACCTv1.buildRedeemMessage(secretA, qortalReceivingAddress);
		String messageRecipient = tradeBotData.getAtAddress();
